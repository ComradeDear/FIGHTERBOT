...
fighter = Var.CUSTOM_STICKER_PACK_NAME


@bot.on(fighter_cmd(outgoing=True, pattern="kang"))
...
async def kang(args):
    ...
    if photo:
        ...
        packname = f"HB_{user.username}_fighter_{pack}"
        packnick = (
            f"{lightning} Vol.{pack}"
            if lightning
            else f"@{user.username}'s fighter Vol.{pack}"
        )
        cmd = "/newpack"
        file = io.BytesIO()
...
@bot.on(fighter_cmd(outgoing=True, pattern="stkrinfo"))
